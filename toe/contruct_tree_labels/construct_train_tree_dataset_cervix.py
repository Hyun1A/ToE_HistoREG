...
if __name__ == '__main__':
    ...
    train_save_path = f"../data/tree_samples/train_cervix.json"

    path = Path(train_save_path)
    path.parent.mkdir(parents=True, exist_ok=True)


    procedures = PROCEDURE_DICT["uterine cervix"]
    cervix_diags = DIAGNOSIS_DICT["uterine cervix"]
    ...
    for idx, item in enumerate(anno_organ):
        ...
        for row in rows_diag:
            ...
            if "-grade squamous intraepithelial lesion " in row:
                samp = generate_sample_case1(samp, item["report"].split(";")[-1][:-1].strip(), row + ")")
            else:
