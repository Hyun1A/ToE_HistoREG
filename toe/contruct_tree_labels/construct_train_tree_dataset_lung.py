...
if __name__ == '__main__':
    ...
    train_save_path = f"../data/tree_samples/train_lung.json"

    path = Path(train_save_path)
    path.parent.mkdir(parents=True, exist_ok=True)


    procedures = PROCEDURE_DICT["lung"]
    lung_diags = DIAGNOSIS_DICT["lung"]
    ...
    for idx, item in enumerate(anno_organ):
        ...
        for row in rows_diag:
            ...
            if "Chronic granulomatous inflammation with necrosis" in row:
                samp = generate_sample_case1(samp, row, "Chronic granulomatous inflammation with necrosis")
                
            elif "Chronic inflammation" in row:
                samp = generate_sample_case1(samp, row, "Chronic inflammation")
            elif "Carcinoid/neuroendocrine tumor, NOS" in row:
                samp = generate_sample_case1(samp, row, "Carcinoid/neuroendocrine tumor, NOS")
            else:
