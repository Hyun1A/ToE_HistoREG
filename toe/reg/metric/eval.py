...
class REG_Evaluator:
    ...
    def evaluate_text_compwise(self, ref_text, hyp_text):
        emb_score   = self.embedding_eval.get_score(ref_text,hyp_text)
        key_score   = self.key_eval.get_score(ref_text,hyp_text)
        bleu_score  = self.get_bleu4(ref_text,hyp_text)
        rouge_score = self.get_rouge(ref_text,hyp_text)
        ...
        ranking_score= 0.15*(rouge_score+bleu_score) + 0.4*key_score + 0.3*emb_score
        return ranking_score, rouge_score, bleu_score, key_score, emb_score
        
    def evaluate_compwise(self, eval_lists):
        ''' list of tuples(pairs) '''
        ranking_score = 0
        rouge_score = 0
        bleu_score = 0
        ...
        pbar = tqdm(eval_lists, total=len(eval_lists))
        for i, (ref_text, hyp_text) in enumerate(pbar):
            ranking_sc, rouge_sc, bleu_sc, key_sc, emb_sc = \
                self.evaluate_text_compwise(ref_text,hyp_text)


            ranking_score += ranking_sc
            rouge_score += rouge_sc
            bleu_score += bleu_sc
            key_score += key_sc
            emb_score += emb_sc

        ranking_score/=len(eval_lists)
        rouge_score/=len(eval_lists)
        bleu_score/=len(eval_lists)
        key_score/=len(eval_lists)
        emb_score/=len(eval_lists)

        return ranking_score, rouge_score, bleu_score, key_score, emb_score
    
    
    def evaluate_compwise_record(self, eval_lists):
        ''' list of tuples(pairs) '''
        ranking_score = 0
        rouge_score = 0
        bleu_score = 0
        ...
        for i, (ref_text, hyp_text) in enumerate(pbar):
            ...
            scores = [rouge_sc, bleu_sc, key_sc, emb_sc, ranking_sc]
            for idx, key in enumerate(score_dict.keys()):
                score_dict[key].append(scores[idx])
        ...
        return ranking_score, rouge_score, bleu_score, key_score, emb_score, score_dict
