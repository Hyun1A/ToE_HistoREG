...
class HierDataset(Dataset):
    ...
    """
    def __init__(self, annotation_path, val_anno_path, raw_feature_path, struct_reports_path, feature_fn, model, mode="train"):
        ...
        self.corrupted_ids = list(pd.read_csv("./reg/corrupt_id/corrupted_id.csv")["id_list"])
        
        for key, val in self.features_all.items():
            if key in self.corrupted_ids or "PIT_04" in key:
                continue
...
class HierDatasetNoisyMixup(Dataset):
    ...
    def __getitem__(self, idx1: int):
        ...
        for idx in idx_list:
            ...
            noise_img = self.lam * image_feature.norm() * torch.randn_like(image_feature) / image_feature.size(1)**0.5
            image_feature = image_feature + noise_img

            noise_txt = self.lam * text_feature.norm() * torch.randn_like(text_feature) / text_feature.size(1)**0.5
...
class HierDatasetOrganwise(HierDataset):
    """
    Minimal skeleton.
    Expect each sample to have:
      - 'feat': 1D array-like of shape [D]
      - (optional) 'organ', 'procedure', 'top_diagnoses', 'diagnosis_node_map'
    """
    def __init__(self, annotation_path, val_anno_path, raw_feature_path, struct_reports_path, feature_fn, model, organ, mode="train"):
        self.raw_feature_path = raw_feature_path
        self.feature_fn = feature_fn
        self.fns = os.listdir(raw_feature_path)
        self.report_model = model
...
class HierDatasetTest(Dataset):
    ...
    Expect each sample to have:
      ...
      - (optional) 'organ', 'procedure', 'top_diagnoses', 'diagnosis_node_map'
    """
    def __init__(self, raw_feature_path, feature_fn, model):
        self.raw_feature_path = raw_feature_path
        self.feature_fn = feature_fn
        self.fns = os.listdir(raw_feature_path)
        self.report_model = model
    ...
    def prepare_features(self):
        if os.path.isfile(self.feature_fn):
            ...
            fns = os.listdir(self.raw_feature_path)

            pbar = tqdm(fns, desc="extract", leave=False)
            for idx, fn in enumerate(pbar, start=1):
                case_id = fn.split(".")[0] + ".tiff"
...
def collate_hier(batch: List[Dict[str, Any]]) -> Dict[str, Any]:
    ids = [b["case_id"] for b in batch]
    image_feats = torch.stack([b["image_feat"] for b in batch], dim=0)
    text_feats = torch.stack([b["text_feat"] for b in batch], dim=0)
    struct_reps = [b["struct_report"] for b in batch]
    reps = [b["report"] for b in batch]
...
def collate_hier_test(batch: List[Dict[str, Any]]) -> Dict[str, Any]:
    ids = [b["case_id"] for b in batch]
    image_feats = torch.stack([b["image_feat"] for b in batch], dim=0)
    text_feats = torch.stack([b["text_feat"] for b in batch], dim=0)
